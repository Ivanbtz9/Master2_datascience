...
import pandas as pd
SNCF = pd.read_csv("C:\\Users\\Admin\\Dropbox\\Enseignement\\Séries Chro M2\\TD-Exam\\Fiche TP\\SNCF.csv", sep=";")
...
plt.plot(X, color="blue")
plt.plot(range(n+1, n+25), Pred01['mean'], linestyle='--', color="orange")
plt.fill_between(range(n+1, n+25), Pred01['mean_ci_lower'], Pred01['mean_ci_upper'], color='orange', alpha=0.2)
plt.xlabel("Mois")
plt.ylabel("Voyageurs de la SNCF (log)")
plt.grid()
...
plt.plot(X, color="blue")
plt.plot(range(n+1, n+25), Pred11['mean'], linestyle='--', color="violet")
plt.fill_between(range(n+1, n+25), Pred11['mean_ci_lower'], Pred11['mean_ci_upper'], color='violet', alpha=0.2)
plt.xlabel("Mois")
plt.ylabel("Voyageurs de la SNCF (log)")
plt.grid()
...
plt.plot(X, color="blue")
plt.plot(range(n+1, n+25), PredHW, linestyle='--', color="green")
plt.xlabel("Mois")
plt.ylabel("Voyageurs de la SNCF (log)")
plt.grid()
...
n = len(SNCF["Voy"])
plt.plot(range(n+1, n+25), np.exp(Pred01['mean'] + Mod01F.params[5]/2), linestyle='--', color="violet")
plt.fill_between(range(n+1, n+25), np.exp(Pred01['mean_ci_lower']), np.exp(Pred01['mean_ci_upper']), color='violet', alpha=0.2)
plt.xlabel("Mois")
plt.ylabel("Voyageurs de la SNCF")
plt.grid()
